...
### "Stream Controller not found at ..." warning
...
- The plugin can't reach the HTTP API at `http://127.0.0.1:8730`
- Check Windows firewall isn't blocking loopback
- Verify port 8730 isn't in use by another app (`netstat -an | findstr :8730`)
...
```json
"Ipc": { "Port": 8731, "BindAddress": "127.0.0.1" }
```
...
Visit `http://127.0.0.1:8730/api/health` in a browser to see:
- Protocol version
