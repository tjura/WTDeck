...
[War Thunder]                          [Stream Controller app]
  ...
  HTTP                                       Local WebSocket SDK
     |                                           |
     v                                           v
[WTDeck.Telemetry] --> [WTDeck.Core Rules] --> [WTDeck.Ipc HTTP :3030] <-- [StreamDock plugin]
                              |                                                   |
                              v                                                   v
...
| WTDeck.Ipc | HTTP REST API bridge (loopback :3030) |
| WTDeck.StreamDock | Plugin installer, profile builder, process controller |
| WTDeck.App | Console host, system tray, DI wiring, sound alerts, sync orchestration |
| WTDeck.Plugin | StreamDock SDK v1 plugin (vanilla JS, WebSocket + HTTP poller) |
...
3. `AppHost` starts as a background service:
   a. Starts `HttpPluginBridge` (listener on 127.0.0.1:3030)
...
8. StreamDock plugin polls `GET /api/stream-dock/state` every 500ms and calls `setImage` + `setTitle`
...
| gears_lamp active + stuck | Damaged | `danger` | Yes | Danger + Sound |
